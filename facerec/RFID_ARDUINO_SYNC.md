...
## ปลดล็อกอัตโนมัติเมื่อจำหน้าได้ (หน้าสแกน)

- ใน `_update_preview()`: ถ้าจำหน้าได้ (กรอบเขียว) **และ** `current_screen == "scan"` เท่านั้น
  → ส่ง `OPEN` ให้ Arduino ปลดล็อกทันที (เหมือนแตะบัตร/กด EXIT)
- มี cooldown `FACE_UNLOCK_COOLDOWN_S = 5.0` วินาที กันส่ง `OPEN` ซ้ำทุก ~150ms ตอนคนยืนอยู่
  หน้ากล้องนิ่งๆ (loop preview ทำงานเร็วกว่านั้นมาก)
- เช็ค `current_screen == "scan"` โดยเจตนา — แม้ตอนอยู่หน้า admin ตัว detection/recognition
  ยังคำนวณอยู่เบื้องหลังเหมือนเดิม (ไม่ได้หยุด) แต่ผลลัพธ์จะ**ไม่มีทาง**ไปสั่งปลดล็อกได้เลย
  ต่อให้จำหน้าใครได้ก็ตาม ต้องกลับมาหน้าสแกนก่อนเท่านั้น
